...
function calculateRollingSpeed(busId, currentSpeed) {
  ...
  return parseFloat(
    (sum / busSpeedBuffer[busId].length).toFixed(2)
  );
}
...
async function processEtaAndUpdate(
  ...
  for (let step = 0; step < allHaltes.length; step++) {
    ...
    if (step === 0) {
      ...
      const prev =
        allHaltes[
          (idx - 1 + allHaltes.length) % allHaltes.length
        ];
  ...
  const hasilSemuaEta = await Promise.all(
    allHaltes.map(async (h, i) => {
      ...
      const eta = await getEtaFromML({
        remaining_halte_count:
          allHaltes.length - r.step,
  ...
  const targetStop = hasilSemuaEta.find(
    (h) => h.is_target
  );
  ...
  emitBusLocation({
    ...
    penumpang:
      payload.passenger_count || 0,

    next_halte_id:
      targetStop?.halte_id || null,

    nama_halte_tujuan:
      targetStop?.nama_halte || null,

    distance:
      targetStop?.distance_meters || 0,

    eta_seconds:
      targetStop?.eta_seconds || null,
...
client.on("message", async (topic, message) => {
  ...
  try {
    ...
    // AMBIL DATA BUS
    // =========================
    const currentBus = await Bus.findByPk(
      bus_id,
      {
        include: [
          {
    ...
    if (
      !activeSchedule?.jalur?.halte?.length
    ) {
      return;
    }
    ...
    // =========================
    let targetIndex =
      busTargetTracker[bus_id] || 0;

    let targetHalte =
      allHaltes[targetIndex];
